<form wire:submit="save" class="space-y-6">
    ...
    <div>
        ...
        <input wire:model="surname" type="text" id="surname" name="surname" required
            class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none" />
        @error('surname')
            <div class="text-red-600 text-sm mt-1">
                {{ $message }}
    ...
    <div>
        ...
        <input wire:model="phone" type="tel" id="phone" name="phone" required
            class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none" />
        @error('phone')
            <div class="text-red-600 text-sm mt-1">
                {{ $message }}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <input wire:model="date" type="date" id="date" name="date" min="{{ now()->format('Y-m-d') }}"
                class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none" />
            @error('date')
                <div class="text-red-600 text-sm mt-1">
                    {{ $message }}
        ...
        <div>
            ...
            <input wire:model="time" type="time" id="time" name="time"
                class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none" />
            @error('time')
                <div class="text-red-600 text-sm mt-1">
                    {{ $message }}
    ...
    <div>
        ...
        <textarea wire:model="note" id="message" name="message" rows="4"
            class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none"
            placeholder="Například speciální požadavky nebo otázky"></textarea>
        @error('note')
            <div class="text-red-600 text-sm mt-1">
                {{ $message }}
            </div>
